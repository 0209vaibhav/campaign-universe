...
function hoverOn(e: React.MouseEvent<HTMLButtonElement>) {
  e.currentTarget.style.borderColor = '#BF4723';
  e.currentTarget.style.color = '#BF4723';
}
...
export default function NodeDetailPanel({ node, onClose, onDelete, onEditClick }: Props) {
      ...
      <div style={{ background: '#161616', border: '1px solid #333' }}>
        ...
        {isSatellite && (
          <div style={{ padding: '0 14px 14px', display: 'flex', gap: '8px' }}>
            <button style={actionBtn} onClick={onEditClick} onMouseEnter={hoverOn} onMouseLeave={hoverOff}>
              Edit Node
            </button>
            <button style={actionBtn} onClick={() => { onDelete(node.id); onClose(); }} onMouseEnter={hoverOn} onMouseLeave={hoverOff}>
              Remove Node
            </button>
          </div>
        )}
